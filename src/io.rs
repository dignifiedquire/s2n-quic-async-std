use std::{convert::TryInto, io, io::ErrorKind, pin::Pin};
...
impl Io {
    ...
    pub fn start<E: Endpoint<PathHandle = PathHandle>>(
        self,
        mut endpoint: E,
    ) -> io::Result<(async_std::task::JoinHandle<()>, SocketAddress)> {
        let Builder {
            rx_socket,
            tx_socket,
            recv_addr,
...
impl<E: Endpoint<PathHandle = PathHandle>> Instance<E> {
    async fn event_loop(self) -> io::Result<()> {
        ...
        let rx_socket = async_fd::AsyncFd::new(rx_socket)?;
        let tx_socket = async_fd::AsyncFd::new(tx_socket)?;
...
mod async_fd {
    ...
    pub struct AsyncFd(Async<std::net::UdpSocket>);
    ...
    pub struct TryIo<'a>(&'a std::net::UdpSocket);

    impl<'a> TryIo<'a> {
        pub fn try_io<R>(
            &mut self,
            f: impl FnOnce(&std::net::UdpSocket) -> io::Result<R>,
        ) -> Result<io::Result<R>, ()> {
            match f(self.0) {
                Ok(v) => Ok(Ok(v)),
